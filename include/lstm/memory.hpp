...
LSTM_BEGIN
    ...
    inline void construct(Alloc & alloc, T * t, Args && ... args)
    {
...
LSTM_DETAIL_BEGIN
    template<typename Alloc, bool = std::is_empty<Alloc>{} && !std::is_final<Alloc>{}>
    ...
    protected:
        constexpr tx_alloc_adaptor_base() = default;
        constexpr tx_alloc_adaptor_base(const Alloc& in_alloc) noexcept(
            std::is_nothrow_constructible<Alloc, const Alloc&>{})
            : Alloc(in_alloc)
        {
        }
        constexpr tx_alloc_adaptor_base(Alloc&& in_alloc) noexcept(
            std::is_nothrow_constructible<Alloc, Alloc&&>{})
...
LSTM_BEGIN
    ...
    private:
        ...
        template<typename U>
        friend struct tx_alloc_adaptor;
    ...
    public:
        ...
        template<typename U, LSTM_REQUIRES_(std::is_constructible<Alloc, U&&>{})>
        constexpr tx_alloc_adaptor(U&& u) noexcept(std::is_nothrow_constructible<Alloc, U&&>{})
            : base((U &&) u)
        {
        }

        template<typename U, LSTM_REQUIRES_(std::is_constructible<Alloc, const U&>{})>
        constexpr tx_alloc_adaptor(const tx_alloc_adaptor<U>& rhs) noexcept(
            std::is_nothrow_constructible<Alloc, const U&>{})
            : base(rhs.alloc())
        {
        }

        template<typename U, LSTM_REQUIRES_(std::is_constructible<Alloc, U&&>{})>
        constexpr tx_alloc_adaptor(tx_alloc_adaptor<U>&& rhs) noexcept(
            std::is_nothrow_constructible<Alloc, U&&>{})
            : base(std::move(rhs.alloc()))
        {
        }

        pointer allocate(size_type n) { return lstm::allocate(alloc(), n); }
        ...
        void deallocate(pointer ptr, size_type n) { lstm::deallocate(alloc(), std::move(ptr), n); }
        ...
        constexpr tx_alloc_adaptor select_on_container_copy_construction() noexcept(
            noexcept(alloc_traits::select_on_container_copy_construction(alloc())))
        {
        ...
        template<typename U>
        constexpr bool
        operator==(const tx_alloc_adaptor<U>& rhs) const noexcept(noexcept(alloc() == rhs.alloc()))
        {
